...
export default function Kanban() {
  ...
  const TaskCard = ({ t }: { t: any }) => {
    ...
    return (
      <div className={`rounded-lg border-l-4 p-3 animate-fade-in ${bgCls}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-2 mt-2">
              {t.status !== 'A Fazer' && (
                <Button variant="secondary" size="sm" onClick={() => handleMove(t.id, 'A Fazer')}>
                  <RotateCcw className="h-4 w-4 mr-1" /> Voltar
                </Button>
              )}
              {t.status !== 'Em Andamento' && (
                <Button variant="outline" size="sm" onClick={() => handleMove(t.id, 'Em Andamento')}>Em Andamento</Button>
              )}
              {t.status !== 'Concluído' && (
                <Button size="sm" onClick={() => handleMove(t.id, 'Concluído')}>
